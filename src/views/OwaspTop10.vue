      ...
      <section class="sej-hero">
        <div>
          ...
          <div class="sej-metrics">
            ...
            <div class="sej-metric"><label>Referensi Pendukung</label><strong>{{ totalAppendices }}</strong><span>Standard mapping (ASVS) dan dokumentasi teknis.</span></div>
            <div class="sej-metric"><label>Rilis</label><strong>2025</strong><span>Versi tahun 2025 terbaru dari OWASP Foundation.</span></div>
          </div>
        </div>
        <div class="sej-side-stack">
          <div class="sej-side"><label>Alur Analisa</label><h3>Pendekatan perlindungan (Shift Left)</h3><p>Pelajari 10 titik kerentanan kritis ini dari pilar kehidupannya (Architecture, Access, dll). Eksplorasi setiap risiko untuk memahami analogi bisnisnya, bagaimana penyerang mengeksploitasi, serta lihat referensi lampiran mitigatif yang tersedia.</p></div>
          <div class="sej-side" style="background: rgba(255, 230, 200, 0.78)"><label><i class="fab fa-creative-commons me-1"></i>Hak Cipta & Lisensi</label><p style="margin-top:0.4rem; font-size:0.8rem">Konten bersumber dari <strong>OWASP Foundation</strong> dan dilisensikan di bawah <a href="https://creativecommons.org/licenses/by-sa/4.0/" target="_blank" rel="noopener noreferrer" style="color:#b45309; font-weight:700;">Creative Commons Attribution-ShareAlike 4.0 International License (CC BY-SA 4.0)</a>. Penggunaan sepenuhnya ditujukan untuk eksplorasi dan mitigasi praktikal internal.</p></div>
        </div>
      </section>

      <div class="sej-nav nav" role="tablist">
        <button class="sej-tab" :class="{ active: activeTab === 'overview' }" type="button" role="tab" @click="activeTab = 'overview'"><i class="fas fa-chart-line"></i><span><strong>Ringkasan Program</strong><span>Peta pilar OWASP, kelompok risiko, dan panduan.</span></span></button>
        <button class="sej-tab" :class="{ active: activeTab === 'explorer' }" type="button" role="tab" @click="activeTab = 'explorer'"><i class="fas fa-sliders-h"></i><span><strong>Eksplorasi</strong><span>Filter risiko Top 10, baca analogi, dan fokus perlindungan.</span></span></button>
        <button class="sej-tab" :class="{ active: activeTab === 'reference' }" type="button" role="tab" @click="activeTab = 'reference'"><i class="fas fa-folder-open"></i><span><strong>Lampiran & Standar</strong><span>Board referensi, cheat sheet, dan pilar ASVS Mapping.</span></span></button>
      </div>
        ...
        <div v-if="activeTab === 'overview'" key="overview-tab">
          <div class="sej-grid two">
            <article class="sej-panel">
              <div class="sej-head"><h3>Kategori pilar keamanan</h3><span class="sej-chip">{{ pillarBreakdown.length }} pilar</span></div>
              <p class="sej-copy">Pilar ini dipakai sebagai panduan utama untuk mengkategorikan kerentanan aplikasi web menurut OWASP. Klik salah satu bar untuk langsung lompat ke eksplorasi dengan filter yang relevan.</p>
              <div class="sej-bars">
            ...
            <article class="sej-panel">
              <div class="sej-head"><h3>Hotspot risiko tertinggi</h3><span class="sej-chip">{{ totalChapters }} risiko</span></div>
              <p class="sej-copy">Visualisasi pemetaan kategori OWASP Top 10 dari urutan keterparahan paling tinggi (peringkat 1 hingga peringkat 10).</p>
          ...
          <div class="sej-grid two">
            <article class="sej-panel">
              <div class="sej-head"><h3>Fokus mitigasi utama</h3><span class="sej-chip">Mitigation focus</span></div>
              <div class="sej-mini-row">
                <div class="sej-mini"><label>Shift Left</label><strong>Desain</strong><span>Menerapkan keamanan sejak awal fase arsitektur dan rancangan.</span></div>
                <div class="sej-mini"><label>Zero Trust</label><strong>Akses & Identitas</strong><span>Validasi ketat pada setiap otentikasi dan kontrol wewenang.</span></div>
                <div class="sej-mini"><label>Defense in Depth</label><strong>Berlapis</strong><span>Pertahanan ganda dari sisi kode, server, hingga lapisan kriptografi.</span></div>
              </div>
              <div class="sej-note">OWASP Top 10 mendorong tim pengembang untuk tidak sekadar menambal bug, tetapi mengadopsi budaya keamanan aplikasi yang holistik dari hulu ke hilir.</div>
            </article>
            <article class="sej-panel">
              <div class="sej-head"><h3>Peta referensi & standar</h3><span class="sej-chip">{{ totalAppendices }} referensi</span></div>
              <p class="sej-copy">Referensi OWASP membantu memperjelas implementasi mitigasi dengan memetakan setiap risiko Top 10 ke standar ASVS dan panduan spesifik seperti Cheat Sheet Series.</p>
              <div class="sej-cards">
        ...
        <div v-if="activeTab === 'explorer'" key="explorer-tab">
          <div class="sej-workspace">
            <article class="sej-panel sej-filter-panel">
              ...
              <div class="sej-form">
                <div><label for="chapterFilter">Peringkat Kerentanan</label><select id="chapterFilter" v-model="chapterFilter" class="form-select"><option value="">Semua</option><option v-for="ch in chapterBreakdown" :key="ch.key" :value="ch.key">{{ ch.key }}: {{ ch.label }}</option></select></div>
            ...
            <article class="sej-panel sej-inspector">
              ...
              <div class="sej-inspector-body">
                ...
                <div class="sej-callout"><span class="sej-label">Panduan Independen & ASVS</span><div class="sej-refs"><button v-for="ref in (activeRequirement?.appendices || [])" :key="ref" type="button" class="sej-ref" @click="jumpAppendix(ref)">{{ ref }}</button><span v-if="!activeRequirement || !activeRequirement.appendices || !activeRequirement.appendices.length" class="sej-empty w-100">Cacat ini tidak menunjuk referensi dokumentasi spesifik ASVS.</span></div></div>
                <div class="sej-note"><span class="sej-label">Skoring Keparahan (OWASP Matrix)</span><div class="mt-2">{{ activeRequirement ? (activeRequirement.scoring || '-') : '-' }}</div></div>
              </div>
            </article>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'reference'" key="reference-tab">
          <div class="sej-refspace">
            <article class="sej-panel">
              ...
              <div class="sej-note mt-3"><span class="sej-label">Hubungan dengan Standar</span>Daftar referensi ini menghubungkan kerentanan tingkat tinggi (Top 10) dengan persyaratan kontrol yang lebih granular agar tim security dapat melakukan remediasi secara terukur.</div>
            </article>
            <article class="sej-panel">
...
export default {
  ...
  data() {
    return {
      ...
      appendices: [],
      pillarMeta: {
        'Architecture': { label: 'Desain & Arsitektur', color: '#1d4ed8', icon: 'fa-sitemap', summary: 'Cacat desain keamanan perangkat lunak.' },
        'Access': { label: 'Akses & Identitas', color: '#0f766e', icon: 'fa-user-lock', summary: 'Masalah kontrol wewenang dan otentikasi.' },
        'Implementation': { label: 'Implementasi Kode', color: '#a16207', icon: 'fa-code', summary: 'Kerentanan logika, injeksi, dan eksekusi server.' },
        'Cryptography': { label: 'Kriptografi', color: '#7c3aed', icon: 'fa-key', summary: 'Kegagalan perlindungan data rahasia.' },
        'Operations': { label: 'Operasional', color: '#b91c1c', icon: 'fa-server', summary: 'Salah konfigurasi, log, dan dependensi.' },
      },
      chapterMeta: {
        'A01': { label: 'Broken Access Control', color: '#b91c1c', icon: 'fa-user-lock', summary: 'Kegagalan akses menempati peringkat 1.' },
        'A02': { label: 'Cryptographic Failures', color: '#7c3aed', icon: 'fa-key', summary: 'Paparan data sensitif dan enkripsi.' },
        'A03': { label: 'Injection', color: '#c2410c', icon: 'fa-syringe', summary: 'SQLi, XSS, OS Command Injection.' },
        'A04': { label: 'Insecure Design', color: '#1d4ed8', icon: 'fa-drafting-compass', summary: 'Cacat sedari blueprint logis aplikasi.' },
        'A05': { label: 'Misconfiguration', color: '#b45309', icon: 'fa-cogs', summary: 'Pengaturan standar yang tidak dikeraskan.' },
        'A06': { label: 'Vulnerable Components', color: '#ef4444', icon: 'fa-layer-group', summary: 'Library pihak ketiga yang memiliki CVE.' },
        'A07': { label: 'Auth Failures', color: '#0f766e', icon: 'fa-id-card', summary: 'Celah login, session, dan kredensial.' },
        'A08': { label: 'Integrity Failures', color: '#4f46e5', icon: 'fa-file-signature', summary: 'Supply chain dan deserialize risks.' },
        'A09': { label: 'Logging Failures', color: '#475569', icon: 'fa-desktop', summary: 'Kegagalan sensor visibilitas SOC.' },
        'A10': { label: 'SSRF', color: '#be185d', icon: 'fa-network-wired', summary: 'Server side fetching abuse.' },
      },
      appendixTypePalette: {
        'Cheat Sheet': '#2563eb',
        'Standard Mapping': '#0f766e',
        'Implementation Guide': '#b91c1c',
      },
      activePillar: '',
      chapterFilter: '',
      requirementSearch: '',
...
.modal-shell{display:flex;flex-direction:column;max-height:85vh;border-radius:20px;overflow:hidden;background:#fff;box-shadow:0 24px 64px rgba(15,23,42,.24),0 8px 24px rgba(15,23,42,.12)}
.modal-sidebar{position:relative;display:flex;flex-direction:row;align-items:center;justify-content:flex-start;gap:.85rem;padding:1rem 1.5rem;color:#fffaf2;text-align:left;flex-shrink:0}
.modal-close{position:absolute;top:50%;transform:translateY(-50%);right:1.25rem;width:2.2rem;height:2.2rem;display:flex;align-items:center;justify-content:center;border:0;border-radius:12px;background:rgba(255,255,255,.2);color:#fff;cursor:pointer;transition:all .2s ease}
