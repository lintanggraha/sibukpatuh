<template>
  ...
  <Transition name="sej-fade">
    <div v-if="selectedAppendix" class="sej-overlay" @click.self="selectedAppendix = null">
      <div class="sej-modal-dialog">
        <div class="sej-modal-shell">
          <div class="sej-modal-sidebar" :style="{ background: `linear-gradient(180deg, ${getAppendixColor(selectedAppendix.type)} 0%, ${getAppendixColor(selectedAppendix.type, 0.72)} 100%)` }">
            <button type="button" class="sej-modal-close" aria-label="Close" @click="selectedAppendix = null"><i class="fas fa-times"></i></button>
            <div class="sej-modal-icon"><i class="fas fa-file-alt"></i></div>
            <div class="sej-modal-id">{{ selectedAppendix.id }}</div>
            <div class="sej-modal-type">{{ selectedAppendix.type }}</div>
          </div>
          <div class="sej-modal-main">
            <div class="sej-modal-header"><h4 class="sej-modal-title">{{ selectedAppendix.title }}</h4></div>
            <div class="sej-modal-body">
              ...
              <section class="sej-modal-section">
                ...
                <ul class="sej-modal-artifact-list"><li v-for="(item, idx) in selectedAppendix.contains || []" :key="idx"><i class="fas fa-check-circle"></i><span>{{ item }}</span></li></ul>
              </section>
              <section class="sej-modal-section">
                <div class="sej-modal-section-header" :style="{ color: getAppendixColor(selectedAppendix.type) }"><i class="fas fa-link"></i><span>Kewajiban Terkait</span></div>
                <div class="sej-modal-requirements">
                  <button v-for="reqId in selectedAppendix.used_by || []" :key="reqId" type="button" class="sej-modal-req-btn" @click="jumpToRequirement(reqId)"><i class="fas fa-arrow-right"></i><span>{{ reqId }}</span></button>
                  <div v-if="!selectedAppendix.used_by?.length" class="sej-modal-empty">Format ini belum dipetakan ke kewajiban khusus.</div>
                </div>
              </section>
            </div>
          </div>
...
.sej-inspector{min-height:720px;display:flex;flex-direction:column}.sej-inspector-head{padding-bottom:.85rem;border-bottom:1px solid var(--line)}.sej-inspector-head strong,.sej-modal-title{display:block;margin-top:.35rem;color:#144e72;font-size:1rem;font-weight:800}.sej-inspector-head span{display:block;margin-top:.28rem;font-size:.9rem;font-weight:800}.sej-inspector-body{display:grid;gap:.75rem;padding-top:.85rem;overflow:auto}.sej-meta,.sej-refs{display:flex;flex-wrap:wrap;gap:.45rem}.sej-plain{margin:.15rem 0 0;padding-left:1rem;color:var(--muted);font-size:.78rem}.sej-empty{padding:.9rem;border:1px dashed rgba(20,38,59,.18);border-radius:16px;background:rgba(255,255,255,.6);color:var(--muted);text-align:center}
.sej-summary{margin-top:.85rem;padding:.82rem .88rem;border-radius:18px;background:linear-gradient(180deg,rgba(25,61,87,.95),rgba(20,78,114,.95));color:#fffaf2}.sej-summary small,.modal-sidebar-type,.sej-modal-section-header{font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em}.sej-summary strong{display:block;font-size:1.72rem}.sej-summary span{display:block;color:rgba(255,250,242,.78);font-size:.78rem}
