<template>
  ...
  <Transition name="sej-fade">
    <div v-if="showReferenceModal" class="sej-overlay" @click.self="showReferenceModal = false">
      <Transition name="modal-slide">
        <div class="sej-modal-dialog" v-if="showReferenceModal">
          <div class="sej-modal-shell">
            <div class="sej-modal-sidebar" :style="{ background: `linear-gradient(180deg, ${getRefTypeColor(selectedReference?.type)} 0%, ${getRefTypeColor(selectedReference?.type, 0.7)} 100%)` }">
              <button type="button" class="sej-modal-close" @click="showReferenceModal = false" aria-label="Close">
                <i class="fas fa-times"></i>
              </button>
              <div class="sej-modal-icon">
                <i class="fas fa-file-alt"></i>
              </div>
              <div class="sej-modal-id">{{ selectedReference?.article || '-' }}</div>
              <div class="sej-modal-type">{{ getRefTypeLabel(selectedReference?.type) }}</div>
            </div>
            <div class="sej-modal-main">
              <div class="sej-modal-header">
                <h4 class="sej-modal-title">{{ selectedReference?.title || 'Detail PBI' }}</h4>
              </div>
              <div class="sej-modal-body">
                <!-- Ringkasan Section -->
                <div class="sej-modal-section">
                  <div class="sej-modal-section-header" :style="{ color: getRefTypeColor(selectedReference?.type) }">
                ...
                <div class="sej-modal-section">
                  ...
                  <div class="modal-section-content">
                    <ul class="sej-modal-artifact-list">
                      ...
                      <li v-if="!selectedReference?.deliverables || !selectedReference.deliverables.length" class="sej-modal-empty">
                ...
                <div class="sej-modal-section">
                  ...
                  <div class="modal-section-content">
                    <div class="sej-modal-requirements">
                      <button v-for="reqId in (selectedReference?.linked_requirements || [])" :key="reqId" type="button" class="sej-modal-req-btn" @click="jumpToRequirement(reqId)">
                        <i class="fas fa-arrow-right"></i>
                        <span>{{ reqId }}</span>
                      </button>
                      <div v-if="!selectedReference?.linked_requirements || !selectedReference.linked_requirements.length" class="sej-modal-empty">
